...
public:
    ...
    std::vector<OthelloBoard*> successor(OthelloBoard* b, char symb);
    void minimaxDecision(OthelloBoard* b, int& col, int& row);
    int minValue(OthelloBoard* b, int& col, int& row, char sym);
    int maxValue(OthelloBoard* b, int& col, int& row, char sym);
